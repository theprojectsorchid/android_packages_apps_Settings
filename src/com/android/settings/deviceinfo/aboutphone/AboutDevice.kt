...
class AboutDevice : FrameLayout {
    ...
    init {
        ...
        val type = SystemProperties.get("ro.lineage.releasetype")

        findViewById<TextView>(R.id.romVersion).text = (if (type == "GAPPS") {
            version + " " + context.getString(R.string.about_device_version_type_gapps2)
        } else {
            version + " " + context.getString(R.string.about_device_version_type_vanilla2)
        }).toString()
